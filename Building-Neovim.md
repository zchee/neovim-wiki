...
To check the translations for language `$LANG`, run `make -C build check-po-$LANG`. Examples:
...
- `check-po-$LANG` generates a detailed report in `./build/src/nvim/po/check-${LANG}.log`. 
  This report is generated by nvim to help debug translation errors, not by `msgfmt`.
